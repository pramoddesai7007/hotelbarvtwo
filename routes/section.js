...
router.post('/create', async (req, res) => {
    ...
    try {
        ...
        // Check the current number of sections
        const sectionCount = await Section.countDocuments();
        if (sectionCount >= 10) {
            return res.status(400).json({ message: 'Cannot create more than 10 sections.' });
        }

        // Create a new section if the count is less than 10
        const newSection = new Section({ name });
        const savedSection = await newSection.save();
        res.status(201).json(savedSection);
    } catch (err) {
